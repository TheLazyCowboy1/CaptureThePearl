...
public partial class CTPGameMode : StoryGameMode
{
    ...
    public void SetupTeams()
    {
        ...
        for (byte i = 0; i < NumberOfTeams; i++)
        {
            try
            {
                tempShelters.Add(Helpers.RandomShelterChooser.GetRespawnShelter(region, SlugcatStats.SlugcatToTimeline(currentCampaign), tempShelters.ToArray(), tempShelters.Count, Plugin.Options.TeamShelterCloseness.Value, Plugin.Options.TargetShelterDistance.Value));
            }
